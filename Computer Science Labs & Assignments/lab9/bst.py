...
class BinarySearchTree:
    ...
    def height(self) -> int:
        ...
        l_counter += max(self._left.height(), l_counter)
        r_counter += max(self._right.height(), r_counter)
        return max(l_counter, r_counter) + 1
    ...
    def items_in_range(self, start: Any, end: Any) -> list:
        ...
        if self.is_empty():
            return []
        elif self._root <= end and self._root >= start:
            lst = [self._root]
        if self._right._root is not None:
            lst += self._right.items_in_range(start, end)
        if self._left._root is not None:
            lst = self._left.items_in_range(start, end) + lst
        return lst
    ...
    def rotate_right(self) -> None:
        ...
        <BLANKLINE>
        """
        self._root, self._right._root, self._left._root, \
        self._right._right._root, self._right._left._root, self._left._right, \
        self._left._left = self._left._root, self._root, \
        self._left._left._root, self._right._root, self._left._right._root, \
        BinarySearchTree(None), BinarySearchTree(None)

        self._right._left, self._right._right = \
        BinarySearchTree(self._right._left._root), \
        BinarySearchTree(self._right._right._root)
    ...
    def rotate_left(self) -> None:
        ...
        self._root, self._right._root, self._left._root, \
        self._right._right._root, self._right._left._root, self._left._right, \
        self._left._left = self._right._root, self._right._right._root, \
        self._root, self._right._left._root, self._left._right._root, \
        BinarySearchTree(None), BinarySearchTree(None)
