...
class Tree:
    ...
    def branching_factor(self) -> float:
        ...
        total_subtrees, total_internal_nodes = self._branching_factor_helper()
        return total_subtrees / total_internal_nodes

    def _branching_factor_helper(self) -> tuple[int, int]:
        """Return a tuple (x,y) where:

        x is total subtrees
        y is total internal nodes
        """
        if self.is_empty():
            return 0, 0
        elif self._subtrees == []:
            ...
            total_internal = 1
            for subtree in self._subtrees:
                child_subtrees, child_internal = subtree._branching_factor_helper()
                total_subtrees += child_subtrees
                total_internal += child_internal
            return (total_subtrees, total_internal)
    ...
    def items_at_depth(self, d: int) -> list:
        ...
        if self.is_empty():
            ...
            for subtree in self._subtrees:
                if d == curr_depth:
                    new_list += subtree.items_at_depth(d - 1)
            return new_list
    # ------------------------------------------------------------------------
    ...
    def insert(self, item: Any) -> None:
        ...
        if self.is_empty():
            ...
            if random == 1 or random == 2:
                ...
                random = randint(0, len(self._subtrees) - 1)
                self._subtrees[random] = self.insert(item)
