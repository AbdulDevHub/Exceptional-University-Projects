...
class BoolOp(Expr):
    ...
    def evaluate(self) -> Any:
        ...
        >>> expr = BoolOp('or', [Bool(True), Bool(True)])
        >>> expr.evaluate()
        True

        >>> expr = BoolOp('and', [Bool(True), Bool(True)])
        >>> expr.evaluate()
        True

        >>> expr = BoolOp('or', [Bool(False), Bool(True)])
        >>> expr.evaluate()
        True
        ...
        """
        if self.op == 'and':
            return all(b.evaluate() for b in self.values)
        elif self.op == 'or':
            return any(b.evaluate() for b in self.values)
        else:
            raise ValueError(f'Invalid operator {self.op}')
...
class Compare(Expr):
    ...
    def evaluate(self) -> Any:
        ...
        if self.comparisons[0][0] == '<':
            truth_list.append(self.left.evaluate()
                              < self.comparisons[0][1].evaluate())
        elif self.comparisons[0][0] == '<=':
            truth_list.append(self.left.evaluate()
                              <= self.comparisons[0][1].evaluate())
        else:
            raise ValueError(f'Invalid operator {self.comparisons[0][0]}')

        for i in range(0, len(self.comparisons) - 1):
            expr = Compare(Num(self.comparisons[i][1].evaluate()),
                           [self.comparisons[i + 1]])
            truth_list.append(expr.evaluate())
