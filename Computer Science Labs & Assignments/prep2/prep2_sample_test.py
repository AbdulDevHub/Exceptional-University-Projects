...
@given(slots=integers(min_value=1), force=integers(min_value=0))
def test_spin_method(slots: int, force: int) -> None:
    """Test that the position of a new spinner is always 0."""
    spinner = Spinner(slots)
    spinner.spin(force)
    assert spinner.position >= 0 and spinner.position < slots

@given(slots=integers(min_value=1))
def test_random_spin_method(slots) -> None:
    """Test that the position of a new spinner is always 0."""
    spinner = Spinner(slots)
